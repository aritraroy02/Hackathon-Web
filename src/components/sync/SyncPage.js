...
const SyncPage = () => {
  ...
  const syncAllRecords = async () => {
    // Note: This is a demo - in offline mode, sync functionality is simulated
    showNotification('Sync completed (demo mode - data stored locally)', 'info');
    ...
    for (let i = 0; i < totalRecords; i++) {
      ...
      try {
        ...
        syncedCount++;
        setSyncProgress((syncedCount / totalRecords) * 100);
      } catch (error) {
    ...
    showNotification(
      `Demo sync completed: ${syncedCount} records processed`,
      'success'
    );
